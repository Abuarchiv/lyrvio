        ...
        <section className="pb-10 border-b border-rule-soft">
          ...
          <ul className="space-y-3 mb-4">
            {[
              ...
              { title: "Cloudflare Workers AI — Llama 3.3 70B (USA/EU):", desc: "Zweck: KI-gestützte Generierung personalisierter Bewerbungsanschreiben im Namen des Nutzers. Verarbeitete Daten: Nutzerprofil-Angaben (Name, Beruf, Suchkriterien) sowie Inserat-Inhalt zur Laufzeit der Generierung — keine dauerhafte Speicherung durch Cloudflare. Rechtsgrundlage: Art. 6 Abs. 1 lit. b DSGVO (Vertragserfüllung). Drittstaatentransfer (USA): auf Basis von EU-Standardvertragsklauseln (SCCs, Art. 46 Abs. 2 lit. c DSGVO). Weitere Informationen: developers.cloudflare.com/workers-ai/privacy" },
            ].map((item, i) => (
              <li key={i} className="font-mono text-[14px] leading-[1.75] text-ink flex gap-3">
                <span className="text-stamp mt-1 flex-shrink-0">■</span>
                <span><strong>{item.title}</strong> {item.desc}</span>
